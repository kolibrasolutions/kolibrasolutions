...
-- Criar o trigger para orders com nome único (se não existir)
do $$
begin
  if not exists (select 1 from pg_trigger where tgname = 'handle_updated_at_orders') then
...
-- Criar o trigger para order_items com nome único (se não existir)
do $$
begin
  if not exists (select 1 from pg_trigger where tgname = 'handle_updated_at_order_items') then
